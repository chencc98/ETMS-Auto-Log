/**
 * 
 */
package org.chencc98.etmslog.utils;

/**
 * @author asus
 *
 */
public class Constants {
	
	public static String ETMS_BASE_URL = "https://www.etms1.astrazeneca.cn/";
	
	public static String ETMS_LOGIN_PAGE = "Login.asp";
	public static String ETMS_EVENTEDIT_PAGE = "Call/EventEditDialog-body.asp";
	public static String ETMS_EVENTINSERT_PAGE = "Call/WeeklyEvent-Updt.asp";
	public static String ETMS_DOCTOR_LOG_PAGE = "Call/CPDocAction-body.asp";
	
	
	public static int MINIMUM_LOG = 20;
	
	public static int A_LEVEL_REQUIRED_LOG = 4;
	public static int B_LEVEL_REQUIRED_LOG = 2;
	public static int O_LEVEL_REQUIRED_LOG = 1;
}
